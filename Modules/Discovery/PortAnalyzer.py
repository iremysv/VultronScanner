...
_PORT_RISK_DB: Dict[int, Tuple[RiskLevel, str, str]] = {
    ...
    512: (
        RiskLevel.CRITICAL,
        "rexec — legacy remote execution (no encryption)",
        "Block port 512. Use SSH for remote execution.",
    ),
    513: (
        RiskLevel.CRITICAL,
        "rlogin — legacy remote login (no encryption)",
        "Block port 513. Migrate to SSH.",
    ),
    514: (
        RiskLevel.CRITICAL,
        "rsh — remote shell (no authentication/encryption)",
    ...
    25: (
        RiskLevel.HIGH,
        "SMTP without AUTH — potential open relay",
        "Require SMTP AUTH; disable unauthenticated relay.",
    ),
    53: (
        RiskLevel.HIGH,
        "DNS — exposed to enumeration / amplification",
        "Restrict recursive queries; implement DNSSEC.",
    ),
    111: (
        RiskLevel.HIGH,
        "RPCbind — exposes RPC services",
        "Block port 111 externally; disable unnecessary RPC services.",
    ),
    135: (RiskLevel.HIGH, "MS-RPC endpoint mapper", "Block port 135 at perimeter firewall."),
    137: (RiskLevel.HIGH, "NetBIOS Name Service", "Disable NetBIOS over TCP/IP if not required."),
    139: (
        RiskLevel.HIGH,
    ...
    8000: (
        RiskLevel.LOW,
        "HTTP dev server — possibly exposed development environment",
        "Ensure not exposed in production.",
    ),
    8888: (
        RiskLevel.LOW,
        "HTTP alternate / Jupyter Notebook",
